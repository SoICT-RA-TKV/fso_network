...
class BlossomBasedGreedy:
    def __init__(self, berDict, berThreshold, hap, RCloud):
        self.berDict, self.berThreshold, self.hap, self.RCloud = berDict, berThreshold, hap, RCloud
    def solve(self):
        ...
        for i in range(NHAP):
            for j in range(NHAP):
                tmpDist = distance(hap[i], hap[j])
                if (tmpDist >= 2*RCloud) and (hap_ber(tmpDist, self.berDict) <= self.berThreshold):
